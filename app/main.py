...
steps_entries = []
ttk.Button(entry_frame, text="Add Step", command=add_step).grid(row=1, column=0, padx=10, pady=5)
ttk.Button(entry_frame, text="Process", command=lambda: process_reaction(steps_entries)).grid(row=2, column=0, padx=10, pady=5)
...
play_button = ttk.Button(guide_frame, text="Play Music", command=lambda: play_notes(steps_entries))
play_button.pack(pady=10)

# Frame for RDKit images
image_frame = ttk.Frame(root)
image_frame.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True, padx=10, pady=10)  # Move to the bottom
